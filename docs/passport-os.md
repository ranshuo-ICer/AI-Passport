...
| 项目 | 优化前 | 优化后 | 加速 |
| --- | --- | --- | --- |
| `_to_be` 1280 字节（字节序翻转） | 6 536 µs | **541 µs** | **12×** |
| `_to_be` 3840 字节 | 19 418 µs | **2 856 µs** | 6.8× |
| `text` 10 字符 | 10 142 µs | **3 213 µs** | 3.2× |
| `text` 28 字符 | 21 956 µs | **4 769 µs** | 4.6× |
| `text_scale` ×3 | 17 111 µs | **6 110 µs** | 2.8× |
| `fill` 整屏 | 69 512 µs | **37 497 µs** | 1.9× |

`fill` 优化后是 37.5 ms，而 153 600 字节在 38.4 Mbit/s 下本身就要 32 ms —— **已经到总线的 85%**。

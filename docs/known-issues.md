...
### #29 多片通知被静默丢弃：`ls` 永远等不到回应（"刷新列表就掉线"的真凶） — `FIXED`

**现象**：手机端一点「刷新列表」就没反应，随后提示掉线 / 让重连；换 PC、换浏览器、
换本地服务、换线上地址**全都一样**。而 `hello` 一直正常。

**根因**：设备端把响应切成多片、**在一个紧凑循环里连续调 `gatts_notify`**。
实测（Windows + bleak，19 个小程序 → `ls` 响应 932 字节 → 6 片）：

```
设备端日志： 响应 932 字节 → 6 片（单包上限 180）      ← 无任何报错
客户端实收： ~181  ~181  ~181  ✗  ✗  !33              ← 第 4、5 片凭空消失
```

**6 次 `gatts_notify` 一次都没抛异常**，但只有 4 片真的到了。这是 ESP-IDF
Bluedroid 的行为：TX 队列满时 `esp_ble_gatts_send_indicate` 照样返回 `ESP_OK`，
包却没了。MicroPython 这一层看不出任何异常。

**为什么它特别难查**：

1. 丢的是**中间**的片（第 4、5 片），头片和 `!` 尾片都在 —— 重组出来的 JSON
   **开头正确、结尾正确、中间缺一段**，`json.loads` 在第 364 字节报
   `Expecting ':' delimiter`。只看"收到几片"根本发现不了。
2. 设备端一片安静（notify 没报错），客户端一片安静（没超时前不打印东西），
   两边日志都"正常"。
3. 它是**后来才坏的**：小程序少的时候 `ls` 响应不足 180 字节、单片就发完了，
   一旦数量涨上去、响应超过一包，就必然踩到。所以看起来像"以前好好的，
   突然就坏了"，而且换任何客户端都不管用 —— 因为问题不在客户端。

**修法**：分片之间留出时间让控制器把 ACL TX 队列排空
（`blepush.py` 的 `_FRAG_GAP_MS = 50`）。同时补上原本缺失的观测能力：
`_write_rsp` 现在会打印"响应 N 字节 → M 片"，并在 notify 失败时打印
第几片、多少字节、什么异常（以前只 `_notify_fail += 1`，从不记录、从不处理）。

**验证**：`tools/` 下的临时探针连发两轮，7 片（1 片 `hi` + 6 片 `ls`）全部到达、
932 字节完整重组、`json.loads` 通过、**19 个 app 且 19 个都带 title**；
正式工具 `python tools/ble_client.py ls` 也能列出全部 19 个程序。

**遗留**：50 ms 是按实测调出来的经验值，不是协议保证。连接间隔（connection
interval）比 PC 更长的手机上可能需要更宽。彻底的做法是让客户端对每个分片回
`ack`（上传通道已经是这个模式），把定时等待换成真正的流控 —— 见「待办」。

### #30 主循环没有错误隔离：任何一条命令出错都会让整个 OS 停机 — `FIXED`

**现象**：设备屏幕停在最后一帧（ST7789 在 MCU 停下后**会保留画面**），看着像
"一切正常、已连接"，但所有客户端都连不上，且**找不到原因**。

**根因**：`ui.py` 的 `run()` 里是裸的 `self.tick()`：
...
穿出去之后被 `main.py` 的 `except` 接住 → 打印 traceback → **停在 REPL**。
整个操作系统下线，而屏幕还挂着旧画面。另外 `blepush.py` 的 `poll()` 里
`self.send({"t":"err"})` 写在 `try` **之外**，发错误响应时再出错也会穿出去。

**修法**：

1. `run()` 的循环包上 try/except：单次异常记 `/crash.log`、打串口、
   **在屏幕上画红底错误页**，然后继续跑；连续失败 20 次才交还给 `main.py`
   （那种情况多半显示/内存已经废了）。
2. `poll()` 里报告错误响应的 `send` 也放进 try。
3. 屏幕留痕这一步最关键 —— 屏幕不写东西的话设备看起来完全正常，
   这是本项目最难查的一类故障，这次就在这上面绕了很久。

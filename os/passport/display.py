...
class Display:
    ...
    def fill_rect(self, x, y, w, h, color):
        ...
        if rows < 1:
            rows = 1
        # 块高不必超过矩形本身。不加这一句，一个 12x6 的小矩形也要铺一块
        # 2040 字节的图案 —— 而这块板铺 2 KB 要 537 µs，比画那 72 个像素贵得多。
        # 更糟的是缓存按 (颜色, 宽度) 存整块，每块约 2 KB，预算 6144 只放得下
        # 3 种宽度，于是画图形（每行一个宽度）时会不停重建。
        # 按 h 截断之后：小矩形几乎免费，缓存条目也小得多，能装下的宽度多一个量级。
        # hline/vline 受益最大（h=1 时块从 1920 字节降到 w*2）。
        if rows > h:
            rows = h
        need = rows * w * 2
